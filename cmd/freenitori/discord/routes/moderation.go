...
func userinfo(context *multiplexer.Context) {
	...
	embed.AddField("Registration Date", snowflake.CreationTime(user.ID).Format("Mon, 02 Jan 2006 15:04:05"), true)
	if member != nil {
...
func guildinfo(context *multiplexer.Context) {
	...
	embed.AddField("Creation Date", snowflake.CreationTime(context.Guild.ID).Format("Mon, 02 Jan 2006 15:04:05"), true)
...
func bulk(context *multiplexer.Context) {
	...
	for _, message := range st {
		if (snowflake.CreationTime(message.ID).Sub(time.Now().UTC()).Hours() / 24) > 14 {
			continue
		}
