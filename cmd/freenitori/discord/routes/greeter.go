...
func welcomeHandler(context *multiplexer.Context) {
	...
	channelID, err := db.GetGuildConfValue(context.Guild.ID, "greet_channel")
	if err != nil {
		return
	}
	if channelID == "" {
	...
	if session.FetchChannel(context.Guild, "", channelID) == nil {
		return
	}
	message, err := db.GetGuildConfValue(context.Guild.ID, "welcome_message")
	if err != nil {
		return
	}
	if message == "" {
	...
	url, err := db.GetGuildConfValue(context.Guild.ID, "welcome_url")
	if err != nil {
		return
	}
	if url != "" {
...
func removeHandler(context *multiplexer.Context) {
	...
	if session.FetchChannel(session.FetchGuild(context.Guild.ID), "", channelID) == nil {
		return
	}
	message, err := db.GetGuildConfValue(context.Guild.ID, "goodbye_message")
	if err != nil {
		return
	}
	if message == "" {
	...
	url, err := db.GetGuildConfValue(context.Guild.ID, "goodbye_url")
	if err != nil {
		return
	}
	if url != "" {
