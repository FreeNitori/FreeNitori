		...
		Entries: []overrides.SimpleConfigurationEntry{
			{
				...
				Cleanup:      func(context *multiplexer.Context) { db.ResetGuildMap(context.Guild.ID, "highlight") },
			...
			},
			{
				...
				Validate: func(context *multiplexer.Context, input *string) (bool, bool) {
					if channel := context.GetChannel(*input); channel != nil {
						*input = channel.ID
						db.ResetGuildMap(context.Guild.ID, "highlight")
						return true, true
					}
					return false, true
				},
...
func handleHighlightReaction(context *multiplexer.Context) {
	...
	channelID, err := db.GetGuildConfValue(context.Guild.ID, "highlight_channel")
	if err != nil {
		return
	}
	amountString, err := db.GetGuildConfValue(context.Guild.ID, "highlight_amount")
	if err != nil {
		return
	}
	...
	e, err := db.GetGuildConfValue(context.Guild.ID, "highlight_emoji")
	if err != nil {
		return
	}
