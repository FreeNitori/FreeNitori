...
func init() {
	...
	overrides.RegisterSimpleEntry(overrides.SimpleConfigurationEntry{
		...
		Format: func(context *multiplexer.Context, value string) (string, string, bool) {
			if value == "" {
				return "Current prefix", config.System.Prefix, true
			}
	...
	overrides.RegisterCustomEntry(overrides.CustomConfigurationEntry{
		...
		Handler: func(context *multiplexer.Context) {
			...
			default:
				err := db.SetCustomizableMessage(context.Guild.ID, context.Fields[2], context.StitchFields(3))
				switch err.(type) {
				default:
					if !context.HandleError(err) {
						return
					}
				case *db.MessageOutOfBounds:
					context.SendMessage(multiplexer.InvalidArgument)
					return
				}
				context.SendMessage("Message `" + context.Fields[2] + "` has been set.")
			case 3:
				err := db.SetCustomizableMessage(context.Guild.ID, context.Fields[2], "")
				switch err.(type) {
				default:
					if !context.HandleError(err) {
						return
			...
			case 2:
				...
				for identifier := range db.CustomizableMessages {
					message, err := db.GetCustomizableMessage(context.Guild.ID, identifier)
					if !context.HandleError(err) {
						return
					}
	...
	case 2:
		for _, entry := range overrides.GetSimpleEntries() {
			if context.Fields[1] == entry.Name {
				...
				embed.Color = multiplexer.KappaColor
				value, err := db.GetGuildConfValue(context.Guild.ID, entry.DatabaseKey)
				if !context.HandleError(err) {
					return
				}
		...
		for _, entry := range overrides.GetSimpleEntries() {
			if context.Fields[1] == entry.Name {
				if context.Fields[2] == "reset" {
					err := db.ResetGuildConfValue(context.Guild.ID, entry.DatabaseKey)
					if !context.HandleError(err) {
						return
					}
				...
				err := db.SetGuildConfValue(context.Guild.ID, entry.DatabaseKey, input)
				if !context.HandleError(err) {
					return
				}
				...
				for _, subEntry := range entry.Entries {
					if context.Fields[2] == subEntry.Name {
						if len(context.Fields) == 3 {
							...
							embed.Color = multiplexer.KappaColor
							value, err := db.GetGuildConfValue(context.Guild.ID, subEntry.DatabaseKey)
							if !context.HandleError(err) {
								return
							}
						...
						if context.Fields[3] == "reset" {
							err := db.ResetGuildConfValue(context.Guild.ID, subEntry.DatabaseKey)
							if !context.HandleError(err) {
								return
							}
						...
						err := db.SetGuildConfValue(context.Guild.ID, subEntry.DatabaseKey, input)
						if !context.HandleError(err) {
							return
						}
